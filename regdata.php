...
//if(isset(email)!=NULL && isset(pass)!=NULL)
//	{	die();	}
...
if (mysqli_connect_errno()){												//to check the connection
	...
	$query = "select count(*) as rows from info where user='".$uuser."'";
    	...
    	$row = mysqli_fetch_array($result)['rows'];
    	if($row == 0){
	//$query = "insert into info values('".$ffname."','".$llname."','".$ssex."','".$eemail."','".$mmob."','".$uuser."','".sha1($ppass)."','EMPTY','EMPTY','EMPTY')";
		$query = "insert into info(fname,lname,sex,email,mob,user,pass) values('".$ffname."','".$llname."','".$ssex."','".$eemail."','".$mmob."','".$uuser."','".sha1($ppass)."');";
